...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            core_1.info(`folderMap: ${JSON.stringify(folderMap)}`);
            for (const each of folderMap) {
                if (each.targetFolder) {
                    core_1.info(`Creating target folder if it doesn't already exist… 📌`);
                    yield io_1.mkdirP(`${temporaryDeploymentDirectory}/${each.targetFolder}`);
                }
                /*
                  Pushes all of the build files into the deployment directory.
                  Allows the user to specify the root if '.' is provided.
                  rsync is used to prevent file duplication. */
                const folderPath = (action.folderPath || '').replace(action.folder, each.folder);
                yield execute_1.execute(`rsync -q -av --checksum --progress ${folderPath}/. ${each.targetFolder
                    ...
                    ? `--delete ${excludes} ${!fs_1.default.existsSync(`${folderPath}/CNAME`) ? '--exclude CNAME' : ''} ${!fs_1.default.existsSync(`${folderPath}/.nojekyll`)
                        ? '--exclude .nojekyll'
                        : ''}`
                    : ''}  --exclude .ssh --exclude .git --exclude .github ${folderPath === action.workspace
